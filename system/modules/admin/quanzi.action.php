...
class quanzi extends admin {
	...
	public function __construct(){
		parent::__construct();
		$this->ment=array(
			array("lists","论坛管理",ROUTE_M.'/'.ROUTE_C.""),
			array("addcate","添加论坛",ROUTE_M.'/'.ROUTE_C."/insert"),
	...
	public function insert(){
		...
		{
			if($_POST['title']==null)_message("论坛名不能为空",null,3);
	...
	public function quanzi_update(){
		...
		if(isset($_POST["submit"])){
			if($_POST['title']==null)_message("论坛名不能为空");
			$title= htmlspecialchars($_POST['title']);
			$guanli= intval($_POST['guanli']);
			$jiaru= $_POST['jiaru'];
			$jianjie=htmlspecialchars($_POST['jianjie']);
