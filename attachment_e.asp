...
If (memName<>Empty And MemCanUP=1) Or (memStatus="8" Or memStatus="7") Then
	...
	IF Request.QueryString("action")="upload" Then
		...
		Else
			...
			Case "GIF"
				Response.Write("<SCRIPT>parent.inputform.Image.value+='\nattachments/"&D_Name&"/"&F_Name&"'</SCRIPT>")
			Case "JPG"
				Response.Write("<SCRIPT>parent.inputform.Image.value+='\nattachments/"&D_Name&"/"&F_Name&"'</SCRIPT>")
			Case "JPEG"
				Response.Write("<SCRIPT>parent.inputform.Image.value+='\nattachments/"&D_Name&"/"&F_Name&"'</SCRIPT>")
			Case "PNG"
				Response.Write("<SCRIPT>parent.inputform.Image.value+='\nattachments/"&D_Name&"/"&F_Name&"'</SCRIPT>")
			End Select
			Response.Write("<SCRIPT>parent.inputform.Image.focus()</SCRIPT><a href='javascript:history.go(-1);'>文件上传成功，请返回继续上传</a><meta http-equiv='refresh' content='1;url=attachment_e.asp'>")
		End IF
		Set F_File=Nothing
		Set FileUP=Nothing
		Response.Write("</td>")
	Else
		Response.Write("<form enctype=""multipart/form-data"" method=""post"" action=""attachment_e.asp?action=upload""><td><input name=""File"" class=""input_bg"" type=""File"" size=""50"">&nbsp;<input class=""submit_bg"" type=""Submit"" name=""Submit"" value="" 确定上传 ""></td></form>")
	End IF
Else
